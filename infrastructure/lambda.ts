...
export const createLambdaFunction = (name, options: {
  filepath: string,
  role: aws.iam.Role,
  handlerName: string,
  ...
  code: new pulumi.asset.FileArchive((async () => {
    await chpExec(`GOOS=linux GOARCH=amd64 go build -o ./dist/functions/${options.filepath}/main functions/${options.filepath}/main.go`);
    await chpExec(`zip -j ./dist/functions/${options.filepath}/main.zip ./dist/functions/${options.filepath}/main`);
